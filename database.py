...
class Database:
    ...
    def init_db(self):
        ...
        # Coins table (for cryptocurrency configuration)
        cursor.execute('''
        ...
        if cursor.fetchone()[0] == 0:
            cursor.execute('''
                ...
                VALUES (60, 0.001, 0, '00:00:00', '23:59:59')
            ''')
        ...
        # Insert default coins if empty
        cursor.execute('SELECT COUNT(*) FROM coins')
        if cursor.fetchone()[0] == 0:
            default_coins = getattr(app_config, 'INITIAL_COINS', [])
            if default_coins:
                cursor.executemany('''
                    INSERT INTO coins (symbol, name, exchange, coingecko_id)
                    VALUES (?, ?, ?, ?)
                ''', default_coins)
    ...
    def get_coins(self) -> List[Dict]:
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM coins ORDER BY created_at DESC, symbol ASC')
        rows = cursor.fetchall()
        conn.close()
        return [dict(row) for row in rows]

    def add_coin(self, symbol: str, name: str, exchange: str, coingecko_id: str) -> int:
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('''
            INSERT INTO coins (symbol, name, exchange, coingecko_id)
            VALUES (?, ?, ?, ?)
        ''', (symbol.upper(), name, exchange.upper(), coingecko_id.lower()))
        coin_id = cursor.lastrowid
        conn.commit()
        conn.close()
        return coin_id

    def delete_coin(self, coin_id: int):
        conn = self.get_connection()
        cursor = conn.cursor()
    ...
    def get_coin_configs(self) -> List[Dict]:
        conn = self.get_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT symbol, name, exchange, coingecko_id FROM coins ORDER BY symbol ASC')
        rows = cursor.fetchall()
        conn.close()
        return [dict(row) for row in rows]

    def get_coin_symbols(self) -> List[str]:
        return [coin['symbol'] for coin in self.get_coin_configs()]
