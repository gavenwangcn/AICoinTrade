...
def add_coin_config():
    ...
    coingecko_id = data.get('coingecko_id', '').strip().lower()

    if not all([symbol, name, exchange, coingecko_id]):
        return jsonify({'error': 'symbol, name, exchange, coingecko_id are required'}), 400

    try:
        coin_id = db.add_coin(symbol, name, exchange, coingecko_id)
        return jsonify({'id': coin_id, 'message': 'Coin added successfully'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/api/coins/<int:coin_id>', methods=['DELETE'])
def delete_coin_config(coin_id):
    try:
